...
public class TaskDiscriptor : MonoBehaviour
{
    ...
    {
        return $"<color=#{textColorCode}>- {task.Discription} <color=#{successCountColorCode}>{task.CurrentSuccess}</color>/{task.NeededSuccessToComplete}</color>";
    }
}
